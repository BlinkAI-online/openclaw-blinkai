...
function resolvePluginConfig(api: OpenClawApi, context?: OpenClawRegisterContext): Record<string, unknown> {
...
function getClientConfig(api: OpenClawApi, context?: OpenClawRegisterContext): BlinkAiClientConfig {
...
export default function register(api: OpenClawApi, context?: OpenClawRegisterContext) {
  api.registerTool({
  ...
  api.registerTool({
    ...
    execute: async (args) =>
      prepareTransaction(getClientConfig(api, context), String(args.action ?? 'unknown'), (args.payload as Record<string, unknown>) ?? {}),
  });
  api.registerTool({
    name: 'blinkai_contract_read',
    description: 'Perform a Blink contract read through BlinkAI.',
    execute: async (args) =>
      blinkRequest(getClientConfig(api, context), '/chain/contract/read', { method: 'POST', body: JSON.stringify({ action: 'contract_read', payload: args }) }),
  });
  api.registerTool({
    name: 'blinkai_post_create',
    description: 'Prepare a Blink feed post creation through BlinkAI.',
    execute: async (args) =>
      blinkRequest(getClientConfig(api, context), '/chain/tx/prepare', { method: 'POST', body: JSON.stringify({ action: 'feed_post_create', payload: args }) }),
  });
  api.registerTool({
    name: 'blinkai_listing_create',
    description: 'Prepare a Blink marketplace listing creation through BlinkAI.',
    execute: async (args) =>
      blinkRequest(getClientConfig(api, context), '/chain/tx/prepare', { method: 'POST', body: JSON.stringify({ action: 'marketplace_listing_create', payload: args }) }),
  });
  api.registerTool({
    name: 'blinkai_tx_submit',
    description: 'Submit a prepared Blink transaction after confirmation.',
    execute: async (args) =>
      blinkRequest(getClientConfig(api, context), '/chain/tx/submit', { method: 'POST', body: JSON.stringify({ action: 'tx_submit', payload: args }) }),
  });

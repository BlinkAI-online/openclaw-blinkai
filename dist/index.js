...
export default function register(api) {
    ...
    api.registerTool({ name: 'blinkai_agent_complete_command', description: 'Complete a claimed Blink user command with the OpenClaw agent response.' }, async (args) => completeAgentCommand(getClientConfig(api), Number(args.commandId), String(args.body ?? ''), (args.metadata ?? {})));
    ...
    api.registerTool({ name: 'blinkai_tx_prepare', description: 'Prepare a BlinkChain transaction with Blink confirmation.' }, async (args) => prepareTransaction(getClientConfig(api), String(args.action ?? 'unknown'), args.payload ?? {}));
    api.registerTool({ name: 'blinkai_contract_read', description: 'Perform a Blink contract read through BlinkAI.' }, async (args) => blinkRequest(getClientConfig(api), '/chain/contract/read', { method: 'POST', body: JSON.stringify({ action: 'contract_read', payload: args }) }));
    api.registerTool({ name: 'blinkai_post_create', description: 'Prepare a Blink feed post creation through BlinkAI.' }, async (args) => blinkRequest(getClientConfig(api), '/chain/tx/prepare', { method: 'POST', body: JSON.stringify({ action: 'feed_post_create', payload: args }) }));
    api.registerTool({ name: 'blinkai_listing_create', description: 'Prepare a Blink marketplace listing creation through BlinkAI.' }, async (args) => blinkRequest(getClientConfig(api), '/chain/tx/prepare', { method: 'POST', body: JSON.stringify({ action: 'marketplace_listing_create', payload: args }) }));
    api.registerTool({ name: 'blinkai_tx_submit', description: 'Submit a prepared Blink transaction after confirmation.' }, async (args) => blinkRequest(getClientConfig(api), '/chain/tx/submit', { method: 'POST', body: JSON.stringify({ action: 'tx_submit', payload: args }) }));
